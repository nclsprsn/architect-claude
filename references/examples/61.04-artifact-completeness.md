...
**Artifact checked:** Phase C Application Architecture — ACME Corp Customer Onboarding (see `23.01-integration-architecture.md`)
...
| 3 | Architecture Principles alignment absent | Invoke `principles-check` skill against the 6 Architecture Principles defined in `11.01-architecture-principles.md`; attach output as an appendix | Head of EA (Marcus Webb) | Before Architecture Board submission | `[proven]` |
...
- **Parallel:** Invoke `principles-check` (item 3) against the 6 Architecture Principles in `11.01-architecture-principles.md`; attach output as an appendix to the Phase C Application Architecture document.
