...
> [!info]
> The SoAW is the contract between the architecture team and its sponsor for a single architecture cycle. It consumes the outputs of the RfAW (`11.02-request-for-architecture-work.md`) and governs all Phase B–G deliverables that follow. Without a signed SoAW the architecture work has no agreed scope and no mechanism to resist scope change. This example follows the 9-clause §7.6 template defined in `skills/architecture-vision/SKILL.md`.
